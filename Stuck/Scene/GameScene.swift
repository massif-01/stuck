...
final class GameScene: SKScene {
    ...
    private func executeMove() {
        ...
        let totalDuration = sequence.reduce(0) { $0 + $1.duration }
        let cycleDuration: TimeInterval = 0.4
        let walkAction = SKAction.customAction(withDuration: totalDuration) { [weak self] _, elapsed in
            guard let self = self else { return }
            let t = elapsed / cycleDuration
            let phase = CGFloat(t - floor(t))
            self.stickFigure.applyPose(.walking(phase: phase))
        }
        stickFigure.run(walkAction)
    }
    ...
    private func executeIdle() {
        ...
        let action = weightEngine.selectAction(from: config.specialActions + [.sit, .stare])
            ?? .stare
        let targetPose = poseForAction(action)
        let duration = Double.random(in: 2...5)

        stickFigure.applyPoseWithInterpolation(target: targetPose, duration: 0.2) { [weak self] in
            self?.stickFigure.run(.wait(forDuration: max(0, duration - 0.2))) { [weak self] in
                self?.isAnimating = false
                self?.scheduleNextBehavior()
            }
        }
